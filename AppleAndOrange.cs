...
class Result
{
    ...
    public static bool hitsHouse(int houseLeft, int houseRight, int tree, int fruit) {
        int fruitLandsAt = tree + fruit;
        return fruitLandsAt >= houseLeft && fruitLandsAt <= houseRight;
    }

    public static void countApplesAndOranges(int houseLeft, int houseRight,
        int appleTree, int orangeTree, List<int> apples, List<int> oranges)
    {
        int applesOnHouse = 0;
        int orangesOnHouse = 0;
        
        foreach (int apple in apples) {
            if (hitsHouse(houseLeft, houseRight, appleTree, apple))
                applesOnHouse++;
        }
        
        foreach (int orange in oranges) {
            if (hitsHouse(houseLeft, houseRight, orangeTree, orange))
                orangesOnHouse++;
        }
